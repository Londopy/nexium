...
registered. Variables, set before the line: `NEXIUM_VERSION=v1.2.1` pins a
...
Variables: `NEXIUM_VERSION=v1.2.1` pins a release, `NEXIUM_HOME` changes the
...
```sh
curl -fsSL https://raw.githubusercontent.com/Londopy/nexium/v1.2.1/bootstrap/nx.c -o nx.c
cc -std=gnu11 -O2 -w -fno-strict-aliasing -o nx nx.c -lm -lpthread
```

(`main` in place of `v1.2.1` gives the seed of the next release, which may
...
```
nx 1.2.2 is available (this is 1.2.1): `nx upgrade` installs it; https://github.com/Londopy/nexium/releases/latest
```
...
```bash
gh attestation verify nx-v1.2.1-x86_64-unknown-linux-gnu.tar.gz --owner Londopy
```
...
```powershell
Get-FileHash .\nexium-1.2.1-setup-x64.exe -Algorithm SHA256
```
