...
```sh
curl -fsSL https://raw.githubusercontent.com/Londopy/nexium/v1.4.1/bootstrap/nx.c -o nx.c
cc -std=gnu11 -O2 -w -fno-strict-aliasing -o nx nx.c -lm -lpthread
```

(`main` in place of `v1.4.1` gives the seed of the next release, which may
...
```
nx 1.4.2 is available (this is 1.4.1): `nx upgrade` installs it; https://github.com/Londopy/nexium/releases/latest
```
...
```bash
gh attestation verify nx-v1.4.1-x86_64-unknown-linux-gnu.tar.gz --owner Londopy
```
...
```powershell
Get-FileHash .\nexium-1.4.1-setup-x64.exe -Algorithm SHA256
```
