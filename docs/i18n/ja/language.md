...
関数は自身のローカルへのスライスやポインタを返せません（規則 R1、1.0 からエラー）。
借用した引数へのビューは呼び出し側が所有するので問題ありません。1.2 はビュー規則
V1 から V5（`SPEC.md` 5.6 と 5.7）を加えます。記憶域より長く保持されたビュー、
コンテナが伸びた後や値が移動した後に使われたビュー、ビューを含む値の返却、
`using arena` ブロックより長く保持された値です。1.2 では警告、1.3 ではエラーで、
`nx check --strict`（または `NX_STRICT=1`）なら今すぐエラーになります。
`@escape(v)` は値をアリーナブロックの外へコピーします。

## 未実装

`soa` と `packed` レイアウト、`node` と `installer` アーティファクト、
`nx publish` とレジストリ、`pool`/`stack` アロケーション戦略、保留された
リージョン規則 R2 から R4（決定 88）。
