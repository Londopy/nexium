...
공격에 강하고 키를 넣은 순서를 지킵니다. `std.time`은 플랫폼의 데이터베이스에서 시간대를 읽고, `std.http`의 클라이언트는 nxtls가 맡는 TLS 계층을 거쳐 HTTPS를 쓰며, `std.websocket`도 같은 계층을 쓰며, [nexium-discord](https://github.com/Londopy/nexium-discord)가 그 위에 Discord 봇을 만듭니다. 다음은 플랫폼의 TLS입니다. Nexium이 아직 아닌 것과 각각이
