...
**Chocolatey y winget**: `choco install nexium` ([el paquete](https://community.chocolatey.org/packages/nexium)) y `winget install Londopy.Nexium`, cada uno en cuanto su registro apruebe la primera versión ([el estado](../../install.md#where-to-get-it), en inglés).
...
| [nxtls](https://github.com/Londopy/nxtls), criptografía en Nexium puro | SHA-2, HMAC, HKDF con las etiquetas de TLS 1.3 y verificación Ed25519, sin C y sin `unsafe`, probados contra los vectores publicados de los estándares; un paquete: `nx add nxtls --git https://github.com/Londopy/nxtls --tag v0.1.0`; un cliente TLS 1.3 es el plan |
...
`std.set`, `std.deque`) y `std.hash` ya están, veintiún módulos en total, y
