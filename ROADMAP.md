...
- `std.text`: grapheme clusters and case mapping tables, `chars()` over
  scalars, width for terminal alignment. (Done: extended grapheme clusters
  (UAX #29) and full case mapping and folding from Unicode 16.0's tables,
  generated by `scripts/unicode_tables.py`; `scalars` gives `char`s
  (`chars` keeps its `u32`s, as 1.x promises); `width` by grapheme cluster
  with `pad_left` and `truncate_width`. Segmentation agrees with ICU but for
  Unicode 17's data changes, case mapping with Python's.)
...
**`nx topo`: the tutorial you can run** (1.4; the runner shipped in
1.2.1).
